...
final class FetchData extends ScheduleEvent {
  ...
  FetchData({required this.groupID, required this.dateStart, required this.dateEnd});
}
...
final class LoadWeek extends ScheduleEvent {
  ...
  LoadWeek({required this.groupID, required this.dateStart, required this.dateEnd});
}
...
final class LoadTeacherWeek extends ScheduleEvent{
  ...
  LoadTeacherWeek({required this.teacherID, required this.dateStart, required this.dateEnd});
}
...
final class ScheduleLoaded extends ScheduleState {
  ...
  ScheduleLoaded({required this.lessons, required this.zamenas, required this.searchType});
}
...
final class ScheduleLoading extends ScheduleState {}
  ...
  ScheduleBloc() : super(ScheduleInitial()) {
    on<LoadTeacherWeek>((event,emit) async {
      ...
      try{
        List<Lesson> lessons = await Api().loadWeekTeacherSchedule(start: event.dateStart, end: event.dateEnd,teacherID: event.teacherID);
        List<Zamena> zamenas = await Api().loadTeacherZamenas(teacherID: event.teacherID, start: event.dateStart, end: event.dateEnd);
        GetIt.I.get<Talker>().critical(zamenas);
        emit(ScheduleLoaded(lessons:lessons,zamenas: zamenas,searchType: CourseTileType.teacher));
      }catch(err){
    ...
    on<FetchData>((event, emit) async {
      ...
      try {
        ...
        emit(ScheduleLoaded(lessons:lessons, zamenas: zamenas, searchType: CourseTileType.group));
      } catch (error) {
        emit(ScheduleFailedLoading());
      }
    });
